...
public class RoverMap {
    ...
    {
        int alternator = 0;
        for(double i = (worldY/-2) + 0.1; i< worldY/2; i+= 3 * scanRange)
        {
            if(alternator == 0)
            {
                for(double j = (worldX/-2) + 0.1; j < worldX/2; j += sqrt(3) * scanRange * 2)
                {
            ...
            {
                for(double j = (worldX/-2) + (sqrt(3) * scanRange) + 0.1; j < worldX/2; j += sqrt(3) * scanRange * 2)
                {
                    System.out.println("Node x: " + Double.toString(i) + " y: " + Double.toString(j));
                    unexploredNodes.add(new Node(i,j));
                }
